...
const ATK = {
    ...
    格斗: { 一般: 2, 岩石: 2, 钢: 2, 冰: 2, 恶: 2, 毒: 0.5, 飞行: 0.5, 超能: 0.5, 虫: 0.5, 妖精: 0.5, 幽灵: 0 },
    ...
    虫: { 草: 2, 超能: 2, 恶: 2, 格斗: 0.5, 飞行: 0.5, 毒: 0.5, 幽灵: 0.5, 钢: 0.5, 火: 0.5, 妖精: 0.5 },
...
function update()
{
    ...
    TYPES.forEach(defType =>
    {
        ...
        selected.forEach(atkType =>
        {
            ...
            if (row)
            {
                mult = Math.max(mult, row[defType] ?? 1);
            }
        });
        if (mult === -1) mult = 1;
